...
#Plot
votu.rec.p <- plot_recruitment("vOTUs")
mag.rec.p <- plot_recruitment("MAGs")
...
yields.p <- yields.tidy %>% 
  ...
  geom_point(size = 1, shape = 16, stroke = 1, alpha = 0.5) +
  ...
  geom_line(data = . %>% group_by(Soil2, Time, Profile2) %>% summarise(MeanYield = mean(Yield)), aes(y = MeanYield), size = 0.75, alpha = 1) +
...
PERMANOVA testing differences between T0 and T6 dry soils. Given the differences in the set of profiles generated for each soil type, we'll use untreated viromes for Jepson, and untreated DNase-treated viromes for Hopland and McLaughlin
```{r}
jpb.dry.map <- filter(v.map, Soil == "JepsonBot", Status == "Dry" & Profile == "ViromeN")
jpt.dry.map <- filter(v.map, Soil == "JepsonTop", Status == "Dry" & Profile == "ViromeN")
hop.dry.map <- filter(v.map, Soil == "Hopland", Status == "Dry" & Profile == "ViromeD")
mcl.dry.map <- filter(v.map, Soil == "McLaughlin", Status == "Dry" & Profile == "ViromeD")

jpb.dry.dist <- v.votu.dist[rownames(v.votu.dist) %in% jpb.dry.map$SampleID, colnames(v.votu.dist) %in% jpb.dry.map$SampleID]
jpt.dry.dist <- v.votu.dist[rownames(v.votu.dist) %in% jpt.dry.map$SampleID, colnames(v.votu.dist) %in% jpt.dry.map$SampleID]
hop.dry.dist <- v.votu.dist[rownames(v.votu.dist) %in% hop.dry.map$SampleID, colnames(v.votu.dist) %in% hop.dry.map$SampleID]
mcl.dry.dist <- v.votu.dist[rownames(v.votu.dist) %in% mcl.dry.map$SampleID, colnames(v.votu.dist) %in% mcl.dry.map$SampleID]

jpb.adonis <- vegan::adonis(as.dist(jpb.dry.dist) ~ TimeFctr,  data = jpb.dry.map)$aov.tab %>% as.data.frame() %>% mutate(Soil = "Jepson Swale", Profile = "Untreated viromes") 
jpt.adonis <- vegan::adonis(as.dist(jpt.dry.dist) ~ TimeFctr,  data = jpt.dry.map)$aov.tab %>% as.data.frame() %>% mutate(Soil = "Jepson Mound", Profile = "Untreated viromes")
hop.adonis <- vegan::adonis(as.dist(hop.dry.dist) ~ TimeFctr,  data = hop.dry.map)$aov.tab %>% as.data.frame() %>% mutate(Soil = "Hopland", Profile = "DNase-treated viromes")
mcl.adonis <- vegan::adonis(as.dist(mcl.dry.dist) ~ TimeFctr,  data = mcl.dry.map)$aov.tab %>% as.data.frame() %>% mutate(Soil = "McLaughlin", Profile = "DNase-treated viromes")

pmanova.supp.table <- rbind(jpt.adonis[row.names(jpt.adonis) == "TimeFctr",],
      jpb.adonis[row.names(jpb.adonis) == "TimeFctr",],
      hop.adonis[row.names(hop.adonis) == "TimeFctr",],
      mcl.adonis[row.names(mcl.adonis) == "TimeFctr",]) %>% 
  mutate(Formula = "dist ~ Timepoint") %>% 
  select(Soil:Formula, Df:"Pr(>F)")

write.table(pmanova.supp.table, "../Tables/supp_table_votu_dry_pmanova.tsv", sep = "\t", quote = F, row.names = F)
```
...
plot_grid(hop.dist.p + theme(legend.position = "none"),
          ...
          get_legend(jpb.dist.p + theme(legend.position = "right")),
          nrow = 1)
```
...
## Get the percent of vOTUs detected in dry soils

dry.detection %>% group_by(OTU_ID) %>% count() %>% nrow() / nrow(votu)
```
...
jac.heatmap.p <- jac.heatmap.df %>% 
  group_by(Soil2.x, TimeFctr.x, TimeFctr.y, Set, Profile2.x) %>% 
...
jac.heatmap.df %>% 
  filter(TimeFctr.x == "T6" & TimeFctr.y == "T1") %>% 
  group_by(Set) %>% 
  summarise(MeanSimilarity = mean(Similarity))
```
