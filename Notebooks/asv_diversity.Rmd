...
v.map <- readRDS("../Data/wetup_map.RDS") %>% 
...
all.dist.tidy <- rbind(mutate(asv.dist.tidy, Set = "ASV"),
                       mutate(v.votu.dist.tidy, Set = "vOTU")) %>% 
  filter(!is.na(Dissimilarity)) %>% 
  filter(SampleID.x != SampleID.y) %>% 
  filter(Soil.x == Soil.y) %>% 
  filter(Profile.x == Profile.y) %>% 
  ...
  mutate(Set2 = str_replace(Set2, "Total MetaG", "Total DNA"))
...
cor_stats_calc <- function(soil.type, set2) {
  dist.cor <- filter(all.dist.tidy, Soil2.x == soil.type & Set == "ASV")
...
dist_plotter <- function(soil.type){
  all.dist.tidy %>% 
    ...
    scale_color_manual(name = "Set", values = profile.pal[c(3,1,2)], 
                       breaks = c("ASVs in Total DNA", "vOTUs in +DNase Virome", "vOTUs in Untreated Virome")) +
...
top <- plot_grid(asv.soil.pcoa.p, asv.richness.p + theme(legend.position = "none"), get_legend(asv.richness.p + theme(legend.position = "right")), 
                 nrow = 1, align = "h", axis = "tb", rel_widths = c(1.3,2.5,1),
